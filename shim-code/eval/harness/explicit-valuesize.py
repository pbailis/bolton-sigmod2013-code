...
resultsdir = "results/explicit-valuesize/"+thetime
...
for variance,  prob_reply, chain_lambda  in [(1, .3, 10.0)]:
    ...
    for iteration in range(0, 5):
        for threads in [2,4,8,16,32,64,128,256]:    
            for valuesize in [4096, 8192, 16384, 32768]:
                ...
                run_lipstick_experiment(False, "%s/E-shim-pgr%f-%d-%dC-%dV-VS%d-%d" % (resultsdir, pgratio, threads, clients, variance, valuesize, iteration), 180)
                
                reset_lipstick_states()
                set_backend_eventual_explicit()
                run_lipstick_experiment(False, "%s/E-eventual-pgr%f-%d-%dC-%dV-VS%d-%d" % (resultsdir, pgratio, threads, clients, variance, valuesize, iteration), 180)
