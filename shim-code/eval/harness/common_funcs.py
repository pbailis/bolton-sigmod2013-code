...
cassandra_root_dir = "/root/apache-cassandra-1.1.0/"
...
def change_cassandra_mem(hosts):
    ...
    run_cmd(hosts, "sed -i 's/4G/4G/' %s/conf/cassandra-env.sh" % (cassandra_root_dir))
    run_cmd(hosts, "sed -i 's/800M/500M/' %s/conf/cassandra-env.sh" % (cassandra_root_dir))
    run_cmd(hosts, "sed -i 's/7G/4G/' %s/conf/cassandra-env.sh" % (cassandra_root_dir))
    run_cmd(hosts, "sed -i 's/hinted_handoff_enabled: true/hinted_handoff_enabled: false/' %s/conf/cassandra.yaml" % (cassandra_root_dir))
    run_cmd(hosts, "sed -i 's/concurrent_reads: 32/concurrent_reads: 128/' %s/conf/cassandra.yaml" % (cassandra_root_dir))
    run_cmd(hosts, "sed -i 's/concurrent_writes: 32/concurrent_writes: 128/' %s/conf/cassandra.yaml" % (cassandra_root_dir))

    run_cmd(hosts, "sed -i 's/concurrent_reads:512/concurrent_reads: 128/' %s/conf/cassandra.yaml" % (cassandra_root_dir))
    run_cmd(hosts, "sed -i 's/concurrent_writes: 512/concurrent_writes: 128/' %s/conf/cassandra.yaml" % (cassandra_root_dir))
...
def set_localstore_kyoto(hosts):
    set_lipstick_shim_param_all(l_hosts, "localstore.class", "edu.berkeley.lipstick.localstore.KyotoLocalStore")

def set_localstore_memory(hosts):
    set_lipstick_shim_param_all(l_hosts, "localstore.class", "edu.berkeley.lipstick.localstore.ConcurrentLocalStore")
...
def gather_lipstick_logs(hosts, outdir):
    ...
    for ip in ips:
        ...
        size =path.getsize(thisdir+"/lipstick.out")
        if(size > 20000000):
            print "Failing on %s" % (thisdir)
            mail("pbailis@cs.berkeley.edu", "too big!", "%d\n%s" % (size, thisdir))
            exit(-1)
        continue
