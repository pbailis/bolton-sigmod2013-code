...
resultsdir = "results/explicit-replyprob/"+thetime
    ...
    for iteration in range(0, 5):
        for prob_reply in [.5, .85, 1.0]:
            ...
            for threads in [2,4,8,16,32,64,128,256]:    
                ...
                run_lipstick_experiment(False, "%s/E-shim-pgr%f-%d-%dC-%dV-pr%f-%d" % (resultsdir, pgratio, threads, clients, variance, prob_reply, iteration), 180)
                
                reset_lipstick_states()
                set_backend_eventual_explicit()
                run_lipstick_experiment(False, "%s/E-eventual-pgr%f-%d-%dC-%dV-pr%f-%d" % (resultsdir, pgratio, threads, clients, variance, prob_reply, iteration), 180)
