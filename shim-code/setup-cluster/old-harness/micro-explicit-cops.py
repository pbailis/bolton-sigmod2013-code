...
for variance,  prob_reply, chain_lambda  in [(1, .3, 10.0), (1, .3, 20.0)]:
    ...
    for iteration in range(0, 5):
        for threads in [2,4,8,16,32,64,128,256]:    
            ...
            run_lipstick_experiment(False, "%s/E-shim-%d-%dC-%fP-%dL-%d" % (resultsdir, threads, clients, prob_reply, chain_lambda, iteration), 180)

            reset_lipstick_states()
            set_backend_strong_explicit()
            run_lipstick_experiment(False, "%s/E-strong-%d-%dC-%dV-%d" % (resultsdir, threads, clients, variance, iteration), 180)

            reset_lipstick_states()
            set_backend_eventual_explicit()
            run_lipstick_experiment(False, "%s/E-eventual-%d-%dC-%dV-%d" % (resultsdir, threads, clients, variance, iteration), 180)

            '''
            reset_lipstick_states()
            set_backend_potential()
            run_lipstick_experiment(True, "%s/P-localcheck-%d-%dC-%dV-%d" % (resultsdir, threads, clients, variance, iteration), 180)
            '''
