...
def runone(runshim, threads, depth, dataset, outfile):
    ...
    system('bin/ycsb load bolton -P workloads/workloada  -p hosts=127.0.0.1 -p fieldlength=%d -p fieldcount=1 -p recordcount=%d -p bolton.explicitdistribution=exponential -p bolton.explicitdistribution.gamma=%s" -threads 10 -s' % (size, records, dataset))
    system('echo "lipstick.pid: \'1\'" >> lipstick.yaml')

    for host in lhosts:
	run_cmd_single_bg(host, "cd /home/ubuntu/ycsb-here/;timeout %d bin/ycsb run bolton -P workloads/workloada  -threads %d -p hosts=127.0.0.1 -p fieldlength=%d -p fieldcount=1 -p operationcount=10000000 -p maxexecutiontime=%d -p recordcount=%d -p bolton.explicitdistribution=exponential -p bolton.explicitdistribution.gamma=%s -s > /tmp/out.txt" % (sec*2, threads, size, sec, records, dataset))
