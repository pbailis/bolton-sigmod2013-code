...
function runone {
    ...
    bin/ycsb load bolton -P workloads/workloada  -p hosts=$CASS_HOST -p fieldlength=$size -p fieldcount=1 -p recordcount=$records -p bolton.explicitdistribution=constant -p bolton.constantdistribution.length=$constant -threads 10 -s
    echo "lipstick.pid: '1'" >> lipstick.yaml
    echo "backend.maxsyncECDSreads:" $depth >> lipstick.yaml
    timeout $(($sec*2)) bin/ycsb run bolton -P workloads/workloada  -threads $threads -p hosts=$CASS_HOST -p fieldlength=$size -p fieldcount=1 -p operationcount=10000000 -p maxexecutiontime=$sec -p recordcount=$records -p bolton.explicitdistribution=constant -p bolton.constantdistribution.length=$constant -s > $1
}

for it in {0..5}
do
    for threads in 1 5 10 20 40
    do
     ...
     do
         ...
         do
          runone IT$it-K$depth-TR$threads-C$constant.txt
         done
     done
    done
done
